...
function updateStateCard(stateName) {
  ...
  // SER block with vs-paper-reference deltas
...
// computeSER is defined in ser.js — we use it here too
function computeSER(state, year) {
  ...
  const R = E > 0 ? S / E : 0;
  const U = E - S;
  return { S: +S.toFixed(6), E: +E.toFixed(6), R: +R.toFixed(6), U: +U.toFixed(6) };
}
